...
exports.ensurePasswordColumn = ensurePasswordColumn;
exports.ensureImageColumn = ensureImageColumn;
exports.ensureVerificationTokenTable = ensureVerificationTokenTable;
exports.ensureShoppingListItemTable = ensureShoppingListItemTable;
exports.ensureShoppingListItemColumns = ensureShoppingListItemColumns;
const prisma_1 = require("./prisma");
async function ensurePasswordColumn() {
    try {
        // For SQLite, check pragma table_info
        const rows = await prisma_1.prisma.$queryRaw `PRAGMA table_info('User')`;
        const has = rows.some((r) => r.name === 'password');
        if (!has) {
            await prisma_1.prisma.$executeRawUnsafe('ALTER TABLE "User" ADD COLUMN password TEXT');
...
async function ensureImageColumn() {
    try {
        const rows = await prisma_1.prisma.$queryRaw `PRAGMA table_info('User')`;
        const has = rows.some((r) => r.name === 'image');
        if (!has) {
            await prisma_1.prisma.$executeRawUnsafe('ALTER TABLE "User" ADD COLUMN image TEXT');
...
async function ensureVerificationTokenTable() {
    ...
    `);
        await prisma_1.prisma.$executeRawUnsafe('CREATE UNIQUE INDEX IF NOT EXISTS "VerificationToken_token_key" ON "VerificationToken"("token")');
    }
    catch (err) {
...
async function ensureShoppingListItemTable() {
    try {
        await prisma_1.prisma.$executeRawUnsafe(`
...
async function ensureShoppingListItemColumns() {
    try {
        const rows = await prisma_1.prisma.$queryRaw `PRAGMA table_info('ShoppingListItem')`;
        const hasNote = rows.some((r) => r.name === 'note');
        if (!hasNote) {
            await prisma_1.prisma.$executeRawUnsafe('ALTER TABLE "ShoppingListItem" ADD COLUMN note TEXT');
