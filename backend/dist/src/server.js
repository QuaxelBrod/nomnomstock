...
const lookup_1 = require("../lib/api/lookup");
const dbFixes_1 = require("../lib/dbFixes");
...
function resolveFileFromCandidates(candidates) {
...
function readTextFileFromCandidates(candidates) {
    const found = resolveFileFromCandidates(candidates);
    if (!found)
        throw new Error(`file_not_found: ${candidates.join(', ')}`);
    return fs_1.default.readFileSync(found, 'utf8');
}
function resolveUploadsDir() {
    const candidates = [
        path_1.default.resolve(process.cwd(), 'frontends', 'web', 'public', 'uploads'),
        path_1.default.resolve(process.cwd(), '..', 'frontends', 'web', 'public', 'uploads'),
...
function resolvePromptTemplatePath() {
    const candidates = [
        path_1.default.resolve(process.cwd(), 'prompts', 'recipe_prompt.txt'),
        path_1.default.resolve(process.cwd(), '..', 'prompts', 'recipe_prompt.txt'),
        path_1.default.resolve(process.cwd(), '..', '..', 'prompts', 'recipe_prompt.txt'),
    ];
...
function getEmailTemplate(name) {
    return readTextFileFromCandidates([
        path_1.default.resolve(process.cwd(), 'emails', name),
        path_1.default.resolve(process.cwd(), '..', 'emails', name),
        path_1.default.resolve(process.cwd(), '..', '..', 'emails', name),
    ]);
}
async function getAuthToken(req) {
    return (0, jwt_1.getToken)({ req: req, secret: process.env.NEXTAUTH_SECRET });
}
async function getAuthContext(req) {
    ...
    const role = typeof token.role === 'string' ? String(token.role) : null;
    ...
    const user = await prisma_1.prisma.user.findUnique({ where: { email }, select: { householdId: true, role: true } });
    return {
        token,
...
function buildHouseholdScope(householdId) {
    if (!householdId)
        return {};
    return { householdId };
}
async function productLookup(barcode) {
    if (!barcode)
        return null;
    const local = await prisma_1.prisma.product.findUnique({ where: { barcode } });
    if (local)
        return local;
    const food = await (0, lookup_1.lookupOpenFoodFacts)(barcode);
    if (food) {
        try {
            return await prisma_1.prisma.product.create({ data: { barcode, name: food.name, brand: food.brand, image: food.image } });
        }
        catch {
            return prisma_1.prisma.product.findUnique({ where: { barcode } });
        }
    }
    const beauty = await (0, lookup_1.lookupOpenBeautyFacts)(barcode);
    if (beauty) {
        try {
            return await prisma_1.prisma.product.create({ data: { barcode, name: beauty.name, brand: beauty.brand, image: beauty.image } });
        }
        catch {
            return prisma_1.prisma.product.findUnique({ where: { barcode } });
        }
...
async function createHouseholdForUser(email, name) {
    ...
    for (let i = 0; i < 20; i += 1) {
        ...
        const existing = await prisma_1.prisma.household.findUnique({ where: { name: candidate } });
        if (!existing) {
            const household = await prisma_1.prisma.household.create({ data: { name: candidate } });
            return household.id;
        }
    }
    const household = await prisma_1.prisma.household.create({ data: { name: `Haushalt ${Date.now()}` } });
    return household.id;
}
...
app.post('/api/auth/credentials', async (req, res) => {
    try {
        ...
        await (0, dbFixes_1.ensurePasswordColumn)();
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        await (0, dbFixes_1.ensurePasswordColumn)();
        await (0, dbFixes_1.ensureVerificationTokenTable)();
        ...
        if (existing && !existing.isActive) {
            await prisma_1.prisma.user.update({
                ...
                data: { name: normalizedName, password: passwordHash },
            });
        }
        else {
        ...
        await prisma_1.prisma.verificationToken.deleteMany({
            where: { email: normalizedEmail, type: { in: ['approval', 'activation'] } },
        });
        let skipSuper = false;
        let invitedHouseholdId = null;
        if (inviteToken) {
            const invite = await prisma_1.prisma.verificationToken.findUnique({ where: { token: inviteToken } });
            ...
            if (invite && isInviteType && (!invite.expiresAt || invite.expiresAt > new Date())) {
                ...
                if (invite.type.startsWith('invite:')) {
                    ...
                    if (Number.isFinite(hh) && hh > 0)
                        invitedHouseholdId = hh;
                }
                skipSuper = true;
                await prisma_1.prisma.verificationToken.delete({ where: { token: inviteToken } });
            }
        }
        if (skipSuper && invitedHouseholdId) {
            await prisma_1.prisma.user.update({
                where: { email: normalizedEmail },
                data: { householdId: invitedHouseholdId },
            });
        }
        else {
            const userAfterUpsert = await prisma_1.prisma.user.findUnique({ where: { email: normalizedEmail } });
            if (!userAfterUpsert?.householdId) {
                const householdId = await createHouseholdForUser(normalizedEmail, normalizedName);
                await prisma_1.prisma.user.update({ where: { email: normalizedEmail }, data: { householdId } });
            }
        }
        if (!emailAuthEnabled || skipSuper) {
            ...
            try {
                ...
                const text = (0, mail_1.renderTemplate)(tpl, {
                    name: normalizedName || normalizedEmail,
                    activateUrl,
                    homeUrl: `${authUrl}/`,
                });
                await (0, mail_1.sendMail)({ to: normalizedEmail, subject: 'Account aktivieren', text });
            }
            catch (mailErr) {
        ...
        const approvalToken = (0, crypto_1.randomBytes)(24).toString('hex');
        await prisma_1.prisma.verificationToken.create({
            data: {
                email: normalizedEmail,
                token: approvalToken,
        ...
        try {
            ...
            await (0, mail_1.sendMail)({ to: superadmin, subject: 'Registrierungsanfrage', text });
        }
        catch (mailErr) {
...
app.post('/api/auth/invite', async (req, res) => {
    try {
        ...
        const inviterUser = await prisma_1.prisma.user.findUnique({ where: { email: auth.email.toLowerCase() } });
        if (!inviterUser)
            return res.status(404).json({ error: 'inviter_not_found' });
        if (!inviterUser.householdId)
            return res.status(400).json({ error: 'no_household' });
        const inviteEmail = normalizeEmail(req.body?.email);
        if (!inviteEmail)
            return res.status(400).json({ error: 'missing' });
        const inviterLabel = `${inviterUser.name || 'User'} <${inviterUser.email}>`;
        const token = (0, crypto_1.randomBytes)(20).toString('hex');
        const tokenType = `invite:${inviterUser.householdId}`;
        await prisma_1.prisma.verificationToken.create({
            data: {
                email: inviteEmail,
                token,
        ...
        try {
            ...
            await (0, mail_1.sendMail)({ to: inviteEmail, subject: 'Einladung zum Vorratsschrank', text });
        }
        catch (mailErr) {
...
app.get('/api/auth/activate', async (req, res) => {
    try {
        ...
        const row = await prisma_1.prisma.verificationToken.findUnique({ where: { token } });
        if (!row || row.type !== 'activation') {
            return res.redirect(`${authBaseFromEnv()}/auth/activated?status=error&reason=invalid`);
        }
        await prisma_1.prisma.user.update({ where: { email: row.email }, data: { isActive: true } });
...
app.get('/api/auth/approve', async (req, res) => {
    try {
        ...
        const actToken = (0, crypto_1.randomBytes)(20).toString('hex');
        await prisma_1.prisma.verificationToken.create({
            data: {
                email: row.email,
                token: actToken,
        ...
        try {
            ...
            const text = (0, mail_1.renderTemplate)(tpl, { name: row.email, activateUrl, homeUrl: `${authUrl}/` });
            await (0, mail_1.sendMail)({ to: row.email, subject: 'Account aktivieren', text });
        }
        catch (mailErr) {
...
app.post('/api/products', async (req, res) => {
    try {
        ...
        if (barcodeRaw) {
            const existingByBarcode = await prisma_1.prisma.product.findUnique({ where: { barcode: barcodeRaw } });
            if (existingByBarcode)
                return res.json(existingByBarcode);
        }
        const existingByName = await prisma_1.prisma.product.findFirst({ where: { name } });
        if (existingByName) {
            ...
            if (isManual) {
                ...
                while (true) {
                    const exists = await prisma_1.prisma.product.findUnique({ where: { barcode: candidate } });
                    if (!exists) {
                        const updated = await prisma_1.prisma.product.update({ where: { id: existingByName.id }, data: { barcode: candidate } });
...
app.post('/api/products/:id/image', async (req, res) => {
    try {
        ...
        await prisma_1.prisma.product.update({ where: { id }, data: { image: imageUrl } });
...
app.delete('/api/products/:id/image', async (req, res) => {
    try {
        ...
        if (product.image && product.image.startsWith('/uploads/')) {
            const uploadsDir = resolveUploadsDir();
            const filename = path_1.default.basename(product.image);
            const filepath = path_1.default.join(uploadsDir, filename);
        ...
        await prisma_1.prisma.product.update({ where: { id }, data: { image: null } });
        return res.json({ ok: true });
    }
    catch (err) {
...
app.post('/api/locations', async (req, res) => {
    try {
        ...
        const location = await prisma_1.prisma.location.create({ data: { name, householdId } });
...
app.patch('/api/locations/:id', async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: 'not found' });
        if (auth.householdId && existing.householdId !== auth.householdId)
            return res.status(404).json({ error: 'not found' });
        const updated = await prisma_1.prisma.location.update({ where: { id }, data: { name } });
        return res.json(updated);
    }
    catch (err) {
...
app.delete('/api/locations/:id', async (req, res) => {
    try {
        ...
        if (auth.householdId && existing.householdId !== auth.householdId)
            return res.status(404).json({ error: 'not found' });
        await prisma_1.prisma.location.delete({ where: { id } });
        return res.json({ ok: true });
    }
    catch (err) {
    ...
    try {
        ...
        for (const stock of stocks) {
            ...
            if (!aggregate.has(key)) {
                ...
                if (new Date(stock.createdAt) < new Date(current.createdAt))
                    current.createdAt = stock.createdAt;
            }
        }
        const aggregated = Array.from(aggregate.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
app.post('/api/stock', async (req, res) => {
    try {
        ...
        const householdId = requestedHousehold || auth.householdId || null;
        const existing = await prisma_1.prisma.stock.findFirst({
            where: {
                productId,
                locationId,
        ...
        if (existing) {
            const updated = await prisma_1.prisma.stock.update({
        ...
        const created = await prisma_1.prisma.stock.create({
            data: {
                productId,
                locationId,
                quantity,
...
app.post('/api/stock/move', async (req, res) => {
    try {
        ...
        const source = await prisma_1.prisma.stock.findUnique({ where: { id: fromStockId } });
        if (!source)
            return res.status(404).json({ error: 'source stock not found' });
        if (auth.householdId && source.householdId !== auth.householdId)
            return res.status(404).json({ error: 'source stock not found' });
        if (source.locationId === toLocationId)
            return res.status(400).json({ error: 'target must be different' });
        ...
        await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (remaining <= 0)
                await tx.stock.delete({ where: { id: source.id } });
            else
                await tx.stock.update({ where: { id: source.id }, data: { quantity: remaining } });
...
app.post('/api/stock/:id/reduce', async (req, res) => {
    try {
        ...
        const stock = await prisma_1.prisma.stock.findUnique({ where: { id } });
        if (!stock)
            return res.status(404).json({ error: 'not found' });
        if (auth.householdId && stock.householdId !== auth.householdId)
            return res.status(404).json({ error: 'not found' });
        ...
        if (toShopping && stock.householdId) {
            await (0, dbFixes_1.ensureShoppingListItemTable)();
            const existing = await prisma_1.prisma.shoppingListItem.findFirst({
                where: { productId: stock.productId, householdId: stock.householdId },
            });
            if (existing) {
                await prisma_1.prisma.shoppingListItem.update({
                    where: { id: existing.id },
                    data: { quantity: existing.quantity + 1 },
                });
            }
            else {
                await prisma_1.prisma.shoppingListItem.create({
                    data: {
                        productId: stock.productId,
        ...
        if (remaining <= 0) {
            await prisma_1.prisma.stock.delete({ where: { id } });
            return res.json({ ok: true, deleted: true });
        }
        const updated = await prisma_1.prisma.stock.update({ where: { id }, data: { quantity: remaining } });
...
async function resolveShoppingHouseholdId(req, auth) {
    const fromQuery = parsePositiveInt(req.query.householdId);
    if (fromQuery && auth.householdId && fromQuery !== auth.householdId)
        return { error: 'forbidden', householdId: null };
...
app.get('/api/shopping', async (req, res) => {
    try {
        ...
        await (0, dbFixes_1.ensureShoppingListItemTable)();
        const items = await prisma_1.prisma.shoppingListItem.findMany({
...
app.post('/api/shopping', async (req, res) => {
    try {
        ...
        if (!name && !productId)
            return res.status(400).json({ error: 'name or productId required' });
        let product = null;
        if (productId) {
            product = await prisma_1.prisma.product.findUnique({ where: { id: productId } });
            if (!product)
                return res.status(404).json({ error: 'product not found' });
        }
        else {
            const barcode = `manual-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
            product = await prisma_1.prisma.product.create({ data: { name, barcode } });
        }
        if (!product)
            return res.status(500).json({ error: 'product not resolved' });
        const existing = await prisma_1.prisma.shoppingListItem.findFirst({
...
app.get('/api/shopping/recent-removed', async (req, res) => {
    try {
        ...
        const history = await prisma_1.prisma.history.findMany({
            where: {
                action: 'REMOVED',
                createdAt: { gte: since },
                householdId: auth.householdId,
        ...
        for (const entry of history) {
            ...
            if (seen.has(entry.productId))
                continue;
            const dismissed = await prisma_1.prisma.history.findFirst({
                where: {
                    productId: product.id,
                    householdId: auth.householdId,
            ...
            if (dismissed && dismissed.createdAt >= entry.createdAt)
                continue;
            const hasStock = await prisma_1.prisma.stock.findFirst({
                where: { productId: product.id, householdId: auth.householdId, quantity: { gt: 0 } },
            });
            if (hasStock)
                continue;
            const onList = await prisma_1.prisma.shoppingListItem.findFirst({
                where: { productId: product.id, householdId: auth.householdId },
            });
            if (onList)
                continue;
...
async function findScopedShoppingItem(id, householdId) {
    if (!householdId)
        return null;
    const item = await prisma_1.prisma.shoppingListItem.findUnique({ where: { id } });
...
app.get('/api/shopping/:id', async (req, res) => {
    try {
        ...
        if (!id)
            return res.status(400).json({ error: 'invalid id' });
        const item = await prisma_1.prisma.shoppingListItem.findUnique({
            where: { id },
            include: { product: true, addedBy: true },
        });
...
app.patch('/api/shopping/:id', async (req, res) => {
    try {
        ...
        if (!item)
            return res.status(404).json({ error: 'not found' });
        await (0, dbFixes_1.ensureShoppingListItemColumns)();
        const data = {};
        ...
        try {
            const updated = await prisma_1.prisma.shoppingListItem.update({ where: { id }, data });
            return res.json({ ok: true, updated });
        }
        catch (err) {
            ...
            if (msg.includes('Unknown argument') && msg.includes('note')) {
                ...
                await prisma_1.prisma.$executeRawUnsafe('UPDATE "ShoppingListItem" SET quantity = ?, note = ? WHERE id = ?', q, n, id);
                const refreshed = await prisma_1.prisma.shoppingListItem.findUnique({ where: { id } });
...
app.delete('/api/shopping/:id', async (req, res) => {
    try {
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.history.create({
                data: {
            ...
            prisma_1.prisma.shoppingListItem.delete({ where: { id } }),
        ]);
        return res.json({ ok: true });
    }
    catch (err) {
...
app.get('/api/profile', async (req, res) => {
    try {
        ...
        try {
            const rows = await prisma_1.prisma.$queryRaw `SELECT image FROM "User" WHERE email = ${email}`;
            if (Array.isArray(rows) && rows[0] && rows[0].image) {
                ;
...
app.post('/api/profile', upload.single('image'), async (req, res) => {
    try {
        ...
        await (0, dbFixes_1.ensureImageColumn)();
        let imagePath = null;
        ...
        if (name) {
            await prisma_1.prisma.user.update({ where: { email }, data: { name } });
        }
        if (imagePath) {
            await prisma_1.prisma.$executeRaw `UPDATE "User" SET image = ${imagePath} WHERE email = ${email}`;
        }
...
app.post('/api/recipes/generate', async (req, res) => {
    try {
        ...
        const template = templatePath
            ? fs_1.default.readFileSync(templatePath, 'utf8')
            : 'Verfügbare Zutaten:\n{{ingredients}}\n\nBenutzerwunsch:\n{{user_input}}\n\nErstelle ein Rezept.';
        const prompt = (0, mail_1.renderTemplate)(template, {
...
app.post('/api/recipes/email', async (req, res) => {
    try {
        ...
        if (!recipient && auth.userId) {
            const user = await prisma_1.prisma.user.findUnique({ where: { id: auth.userId } });
            recipient = user?.email || null;
        }
        if (!recipient)
            return res.status(400).json({ error: 'no email available' });
        const text = recipe;
        const html = `<pre style="white-space:pre-wrap">${recipe.replace(/</g, '&lt;')}</pre>`;
        await (0, mail_1.sendMail)({ to: recipient, subject, text, html });
        return res.json({ ok: true });
    }
    catch (err) {
...
app.post('/api/debug/login', async (req, res) => {
    try {
        ...
        const hash = user.password;
        if (!hash)
            return res.status(403).json({ ok: false, error: 'no_password' });
