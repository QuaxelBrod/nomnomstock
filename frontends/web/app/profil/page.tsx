...
export default function ProfilPage() {
  ...
  const onInvite = async (e: React.FormEvent) => {
    ...
    setInviting(true)
    try {
      const res = await fetch(`${base}/api/v1/auth/invite`, {
        method: 'POST',
    ...
    <main className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      {session ? (
        <div className="mt-4">
          ...
          <div className="mt-3">
            <button onClick={() => signOut({ callbackUrl: '/auth/login' })} className="action-fullmobile px-3 py-1 text-sm text-red-600 border rounded">Logout</button>
          </div>
